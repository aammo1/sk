...
| field | value |
|---|---|
| question | "This folder isn't empty. Where do you want the starter created?" |
| header | "Folder strategy" |
| options | See below |
| multiple | false |

Options:
- **Subfolder (Recommended)** — pick a name (default: `my-app`); the starter lives at `./<name>/`.
...
If the user picks **Subfolder**: ask (with a follow-up `question` call) for the folder name, default to `my-app`. The scaffold target in Step 4 becomes `./<name>`. Skip the temp-dir dance.
...
3. After scaffolding completes and the rest of the steps that *can* be done inside the temp dir are done (env file, migrations, etc. — they all work from inside the temp dir), move everything back. Use a command that includes hidden files. POSIX:
   ...
   shopt -u dotglob
   ```
   This catches `.env`, `.gitignore`, `.agents/`, `.opencode/`, and any other dotfiles the starter ships.
4. **Before each overwrite**: if a file in the temp dir collides with an existing file in the cwd (e.g. the user already has a `README.md`, a `.gitignore`, or a `.git/` directory), stop and ask the user via the `question` tool which one to keep. Do not silently clobber their work.
...
Ask via the `question` tool, but only show options that succeeded in Step 1's probe:

| field | value |
|---|---|
| question | "Which package manager should the starter use?" |
| header | "Package manager" |
| options | See below — only include options where the tool was detected |
| multiple | false |

Options (only show detected ones):
- **pnpm (Recommended)** — only if `pnpm --version` succeeded.
- **npm** — only if `npm --version` succeeded.
- **yarn** — only if `yarn --version` succeeded.
...
| field | value |
|---|---|
| question | "Which PostgreSQL setup do you want?" |
...
Ask once via the `question` tool (multi-select):

| field | value |
|---|---|
| question | "Which production integrations do you want set up now? Pick any that apply, or leave them all unchecked to skip and keep the setup lean — you can add them later." |
| header | "Production extras" |
| multiple | true |
...
Start the dev server in the background and capture its output to a log file:

```bash
<pm> dev > /tmp/opencode/dev-server.log 2>&1 &
echo "PID=$!"
```

Next.js needs a few seconds to compile and bind to port 3000, so poll for readiness:

```bash
for i in $(seq 1 15); do
  if grep -qE "Ready in|Local:.*http://localhost:3000" /tmp/opencode/dev-server.log 2>/dev/null; then
    echo "Dev server is ready"
    break
  fi
  if [ $i -eq 15 ]; then
    echo "WARNING: Dev server did not report ready. Checking anyway..."
  fi
  sleep 2
done
```
...
1. **HTTP 200 from `http://localhost:3000`:**
   ```bash
   curl -s -o /dev/null -w "%{http_code}\n" http://localhost:3000
   ```

2. **No fatal errors** in the dev server log — scan for stack traces, DB connection refusals, or missing-env-var warnings:
   ```bash
   grep -iE "error|fatal|cannot|refused" /tmp/opencode/dev-server.log || echo "No errors found"
   ```

3. **The process is still alive:**
   ```bash
   pgrep -f "next dev" > /dev/null && echo "Dev server process is running" || echo "WARNING: Dev server process not found"
   ```

If any of these fail, common causes:
...
Use the project instructions in AGENTS.md or .opencode/ configuration. During planning, ask clarifying questions before making assumptions. During implementation, split the work into small chunks, use sub-agents where useful, follow DESIGN.md for UI, preserve the existing tech stack unless there is a good reason to change it, and run lint, typecheck, and build before finishing.

What I want to build:
[Describe your app here]
```

4. **A pointer** to the project instruction files: `AGENTS.md`, and `DESIGN.md` (shipped by the starter). Also note that opencode reads project-level configuration from `.opencode/` and user-level from `~/.config/opencode/` — useful if the user wants to customize opencode's behavior for this project.

Keep this final message short — the user wants to start building, not read a wall of text.

## Publishing Note

If the user asks how other people can install this skill, the answer depends on the platform:

**For opencode:** Copy the skill directory into the skills folder:
```bash
